...
import { GiftIcon, TrashIcon, PencilSquareIcon, PlusIcon, TicketIcon } from '@heroicons/react/24/outline';
import Modal from '../common/Modal';

const RedemptionSettings = () => {
  const { items, isLoading, addItem, updateItem, deleteItem } = useRedemptionItems();
  const { coupons } = useCoupons(); // Fetch coupons for selection
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const itemData = { title, points, colorTheme, isActive, linkedCouponId };
      if (editingItem) {
        ...
        {items.map(item => (
          <div key={item.id} className={`p-4 rounded-xl border-2 flex items-center gap-4 relative group ${item.isActive ? 'bg-white border-gray-100' : 'bg-gray-50 border-dashed border-gray-200 opacity-60'}`}>
             <div className={`w-12 h-12 rounded-full flex items-center justify-center shrink-0 ${
                 themeColors[item.colorTheme].split(' ').filter(c => !c.startsWith('border')).join(' ')
             }`}>
