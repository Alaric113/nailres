...
const BookingPage = () => {
  ...
  // Season Pass State (Pass actingUserProfile) - MUST be before early return
  ...
  // Auto-select pass if available and not set - MUST use useEffect and be before early return
  useEffect(() => {
  ...
  // Get booking info for selected designer - MUST be before early return
  const {
  ...
  // Auto-select pass if available and not set (moved after loading check is safer)
  // This is actually a side effect, should use useEffect not useMemo
  // But for now keeping useMemo to avoid changing behavior
  ...
  */
  ...
  return (
    <div className={`${isLiff ? 'min-h-[100dvh]' : 'min-h-[calc(100vh-64px)]'} bg-[#FAF9F6] pb-20`}>
