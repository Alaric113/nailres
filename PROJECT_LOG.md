...
- **優惠券系統 (後台):**
    - 建立了完整的後台管理介面 (`PromotionsPage.tsx`)。
    - 支援新增、編輯、查看優惠券列表 (`CouponForm`, `CouponList`)。
    - 具備向特定或全體用戶發送優惠券的功能 (`CouponDistribution`)。
- **集點卡系統 (後台):**
    - 提供了後台設定集點規則的介面 (`LoyaltySettings`)。

### **2.7. 作品集與案例展示 (Portfolio & Showcase)**
- **作品集管理與展示:**
    - 建立了作品集項目資料模型 (`portfolioItems`)。
    - 開發了後台管理介面 (`PortfolioManagementPage.tsx`)，支援作品的新增、編輯、刪除。
    - 整合了圖片上傳功能 (`ImageUploader.tsx`)，支援多張圖片管理。
    - 建立了客戶端作品集展示頁面 (`PortfolioGalleryPage.tsx`)，支援分類篩選。
...
- **[ ] ⭐️ 客戶評價與回饋系統 (Customer Reviews):**
    - **目標:** 建立客戶信任感並收集回饋。
    - **自動化:** 訂單完成後，自動發送 LINE 訊息邀請客戶評分。
    - **前台:** 在服務頁面或「客戶見證」頁面顯示精選評價。

- **[ ] 📅 增強版預約提醒 (Enhanced Reminders):**
    - **目標:** 降低客戶預約未到率。
    - **功能:** 在預約成功頁面與 LINE 提醒中，新增「加入行事曆」按鈕 (產生 `.ics` 檔案)。

- **[ ] ❓ 常見問題頁面 (FAQ Page):**
    - **目標:** 節省客服時間。
    - **功能:** 建立 `/faq` 靜態頁面，分類整理常見問答 (如預約需知、交通資訊、取消政策等)。

- **[ ] 🚀 架構與效能優化 (Architecture & Performance):**
    - **目標:** 提升系統長期穩定性與使用者體驗。
    - **任務:**
        - **數據統計強化:** 建立 Cloud Function 彙總每日/月營收、熱門服務等數據，並在後台儀表板以圖表呈現。
        - **前端效能優化:** 於 `App.tsx` 路由中導入 `React.lazy()` 實現程式碼分割 (Code Splitting)。
        - **導入 Storybook:** 將現有元件納入 Storybook，建立可互動的元件庫。
        - **建立 E2E 測試:** 針對預約、登入等核心流程，引入 Cypress 或 Playwright 撰寫端到端測試。
